...
public class AnalisisTexto {
    ...
    private final String textoAnalizar = """
    ...
    public void accesoNivelBasico() {
        patron = Pattern.compile("[A-Z]{3}-[0-9]{4}");
    ...
    public void accesoPriviligeado() {
        patron = Pattern.compile("(ROOT|ADMIN|X-ADMIN)-[0-9]{4}");
        matcher = patron.matcher(textoAnalizar);
        while (matcher.find()) {
            System.out.println(matcher.group());
        }
    ...
    public void identificadoresInternos() {
        patron = Pattern.compile("SYS_[A-Z]{3}[0-9]{2}");
        matcher = patron.matcher(textoAnalizar);
        while (matcher.find()) {
            System.out.println(matcher.group());
        }
